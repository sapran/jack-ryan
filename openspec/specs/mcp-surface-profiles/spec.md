# mcp-surface-profiles Specification

## Purpose

Defines which tools the surface advertises to whom: named profiles with
explicit allow-sets that narrow rather than widen on a mistake, and one table
that stamps every tool by its worst reachable mode.
