# casefile-lifecycle Specification

## Purpose

Defines what a casefile is — the container everything an investigation holds
will belong to — how it is named, how a reference to one resolves, and the
guarantee that its identity survives renaming.
...
description, and creation and update timestamps.

The identifier SHALL be stable for the life of the casefile, so that references
held elsewhere stay valid across renames.

The casefile is the scope every later object belongs to — that is why it exists
before anything it will contain — but no such object exists yet. That commitment
is recorded in `docs/design.md` § 4 and becomes normative in the capability that
introduces each one.
