...
- [x] 6.3 Ingest the legacy tail of the real dump and record the document count and media-type breakdown; verify every legacy file becomes a document with the legacy media type, since these files were previously invisible rather than failed. *Amended and deliberately narrowed. The task as written called for the whole 1922-file dump and a count rising against the 1502 it produced before. That run was started twice and abandoned: it is dominated by a cost this change does not touch — one 6.8 MB workbook in the dump extracts to 8.9 MB of text, about a sixth of the entire corpus, and spends over twenty minutes being chunked and embedded. Re-establishing the 1502 baseline measures the embedder, not this change. Scoped instead to the 259 files this change is about, which isolates the variable: **258 ingested, 1 failed** in 34m56s, 3316 chunks, 5,716,813 characters that were previously unreachable. The one failure is the HTML-misnamed-`.xls`, refused with `is named .xls but is neither an OLE2 nor an OOXML container` — the predicted file, the predicted message. Media types: `application/msword` 168, `application/vnd.ms-excel` 81, `application/vnd.ms-powerpoint` 8, `application/rtf` 1 — every one the legacy type, none the converted one. Lineage: `legacy-office+docling` 177, `legacy-office+spreadsheet` 79, `legacy-office-passthrough+spreadsheet` 2 — the two rescued mislabels, and no third literal. All 258 report `text_source` `native`. A search over that casefile returns cited passages out of converted `.doc` files, so the whole loop closes.*
