...
- [ ] 3.4 Raise `ExtractionError` naming the archive when the reader cannot open it, covering the encrypted case; verify a test with an encrypted fixture asserts the message names encryption and that no document with zero children is stored — for both generations of the format, both password modes, and both passes, since libarchive delivers a data-encrypted RAR3 entry as ciphertext without an error
- [ ] 3.5 Raise `ExtractionError` naming multi-volume explicitly rather than letting libarchive list a first volume as a whole archive; verify a test asserts a volume built to the format — its main header's volume flag, or an entry declaring that its data continues elsewhere — is refused under a filename carrying no ordinal, since deciding this on the name refuses whole archives and reads renamed volumes
- [ ] 3.5a Raise `ExtractionError` when the header walk establishes that an archive's own headers do not account for the file; verify a test asserts each truncated shape names why, and that an archive which genuinely holds nothing is still stored as a container with no children
- [ ] 3.5b Put the pre-open refusals behind one function called by `extract()` and by `iter_children()`, and ask libarchive's per-entry encryption verdict in both loops; verify a test drives `iter_children()` alone on an encrypted archive and on a volume, since the pass that reads an entry's bytes is not the pass whose refusal is reported
...
- [ ] 3.8 Yield `Child(name=entry.pathname, data=...)` one entry at a time, reading each entry's blocks before advancing the cursor; verify a test asserts the later entries' bytes were never delivered by the reader at the moment the first child is yielded — a test that only takes one child and closes the generator passes for an implementation that reads the archive into a list and `yield from`s it — and that abandoning it partway does not raise
