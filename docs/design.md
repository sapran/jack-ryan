...
3. Vector store: **resolved — sqlite-vec** (one store, one backup, transactional; the `StorePort` seam covers a later swap).
...
8. Analyst roster: how many legs (2–4) and their exact operating questions — corpus / entity / timeline-context / document-forensics is a starting cut, not settled. Firm up before M4, when the roles are authored.
9. Competency-grid method: **resolved — not adopted.** Skills are a curated set hand-authored by maintainers/JR devs (AI-assisted, never AI-autonomous), so the invention risk the grid guards against is removed at the source; evidence-chain traceability (claims → documents via Citations) is the enforced must instead. See §6.
...
11. Extraction spike specifics (M1): docling OCR engine choice and config for eng+ukr+rus (EasyOCR default vs alternatives — upstream Cyrillic support requires explicit configuration), where the VLM pipeline earns its cost, and what the offline-fallback floor guarantees.
