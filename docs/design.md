...
2. *Enrich* — per-chunk contextual summaries (LLM; the dominant ingest cost, since a large dump is millions of LLM calls, so this is a **contract** switch — corpus-coupled, because folding a summary into what is embedded changes what the vector means, so enabling it refuses an existing corpus and requires a reingest — **off in the prototype** for fast/cheap ingest and enabled later for retrieval quality); a per-document summary cascade; NER + pattern extraction → mentions; a metadata cascade (LLM-first, deterministic fallbacks, per-field provenance). *(The whole enrich layer beyond chunk+embed — summaries, mentions/NER, metadata — is deferred to M3; the prototype extracts, chunks, and embeds.)*
...
- **Summarization layer** — per-chunk contextual summaries and per-document summaries → **M3** (corpus-coupled: enabling it invalidates an existing corpus, so the switch and the summariser's identity are contract values).
