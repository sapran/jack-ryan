...
2. *Enrich* — per-chunk contextual summaries (LLM; the dominant ingest cost, since a large dump is millions of LLM calls, so this is a **corpus-coupled** switch — folding a summary into what is embedded changes what the vector means, so enabling it refuses an existing corpus and requires a reingest. It lives in the `profiles` layer and enters corpus identity by **composition**, exactly as `profiles.<name>.embedder` does, rather than being declared in the `contract` block: the summariser's identity is its model plus a hash of the prompt, the truncation limit and the sampling parameters, all of which live in shipped code, so an operator cannot know the value and a declared copy could disagree with the code. **Off in the prototype** for fast/cheap ingest and enabled later for retrieval quality); a per-document summary cascade, which writes rows and moves no vector, so it is outside corpus identity and can be turned on over an existing corpus; NER + pattern extraction → mentions; a metadata cascade (LLM-first, deterministic fallbacks, per-field provenance). *(Summaries shipped in M3; mentions/NER and the metadata cascade remain.)*
...
**Search.** Casefile-scoped always. Hybrid keyword + vector retrieval fused by reciprocal-rank fusion → cross-encoder rerank (a scoring failure degrades to unranked; a named model that cannot be built is fatal, and none is named by default — see `docs/handover.md` for why) → expansion of a matched chunk to a coherent section-sized window for the agent to read. Facets: doc type, language, date, path/container, mention, tag, actor. The read stack runs offline with zero configured endpoints. *(The prototype ships fusion only; rerank and section-window expansion are deferred to M3. The **mention facet is built** — an inventory of extracted identifiers, and a filter that narrows a search to passages carrying one, applied by the retrievers rather than to their results so that a bounded candidate depth cannot hide a match. Tag and actor facets still arrive with the features that populate them.)*
...
- ~~**Summarization layer**~~ — per-chunk contextual summaries and per-document summaries → **shipped in M3** (corpus-coupled: turning the fold on invalidates an existing corpus. The switch stays in the `profiles` layer and the summariser's identity is *composed* into corpus identity rather than declared in the `contract` block, because that identity is partly a hash of shipped code — see the `contextual-summaries` change. The per-document summary moves no vector and is outside corpus identity).
