...
- **Verifying a WAL-mode migration backup either fails or leaves sidecars beside the artefact being verified, depending on the client.** `backup_before_migrating` copies through SQLite's backup API, which carries the source's WAL mode into the copy's header while writing no `-wal` or `-shm` beside it. Isolated on 2026-09-09 against a copy of a real `.v10.bak`, running each client against an untouched file in a known order: the `sqlite3` CLI (Apple's 3.51.0) with `-readonly` refuses it outright — `SQLITE_CANTOPEN`, reported as "unable to open database file (14)" at statement preparation — and creates nothing; Python's `sqlite3` with `file:…?mode=ro` opens the same untouched file successfully and *creates* both a `-shm` and an empty `-wal`; the CLI then succeeds on the third attempt precisely because the `-shm` now exists. So the refusal is a property of the client, not of the URI mode — an earlier version of this entry blamed `mode=ro` and was wrong — and the operator told "a copy of the store as it was is beside it" is choosing between a client that will not read it and a client that modifies its directory to do so. `immutable=1` reads it with no side effects at all, and is the right tool here, but only once a non-empty `-wal` has been ruled out, because that flag makes SQLite ignore one and return stale content silently. All four `.vN.bak` files proved genuine that way: each recorded the `schema_version` its name claims, `quick_check ok`, equal document and mention counts. Compounds the zero-byte entry above — one code path both accepts an invalid backup and gives no unambiguous way to audit a valid one. Parked for the same backup-validity change.
- **Rungs can accumulate in an uncheckpointed WAL, leaving the database file itself several versions stale.** Observed on 2026-09-09: `data/jackryan.db` recorded `schema_version=7` in the file while the store answered `11`, with rungs 8 to 11 and every write since living entirely in a 19 MB `-wal`. A `serve-mcp` connection had been open continuously for twenty hours, and a long-lived reader blocks the checkpointer. Two consequences matter before anyone copies a store: a plain file copy yields a four-rung-stale database missing `document_places` outright, and losing the `-wal` reverts the migrations together with the data. SQLite is behaving as documented — but the ladder's backups and its reassurance message both speak of "the store" as if it were one file, and the ladder takes a `.bak` per rung while the file it is protecting may be far behind. Parked as an operator-guidance gap, not a defect in the ladder.
- **Two mention spans can legitimately share a document position once their offsets are correct.** After `repair mention-offsets` ran over a real corpus on 2026-09-09, exactly one position carried two overlapping `email` spans, of 24 and 27 characters, sharing a start; before the repair they differed only because one offset was wrong. Per-facet counts are unaffected — `retrieval.py` counts distinct positions within a single `(kind, normalised)` group — and no shipped surface reports a corpus-wide distinct-position figure. But such a figure is therefore *not* the sum of the per-facet counts, which is a trap for anyone writing a reconciliation check over the whole table. The overlapping pair belongs to the extractor, not the repair. Parked; no surface is wrong today.
